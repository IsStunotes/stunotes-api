...
public class ReminderServiceUnitTest {
    ...
    void testCreateReminder_Success() {
        ...
        ReminderResponse response = new ReminderResponse(1L, "Test Reminder", "Activity Name", request.dateTime());
    ...
    void testCreateReminder_NoActivityId_Success() {
        ...
        Reminder reminder = new Reminder();
        ReminderResponse response = new ReminderResponse(1L, "Test Reminder", null, request.dateTime());

        when(reminderMapper.toEntity(request, null)).thenReturn(reminder);
        when(reminderRepository.save(reminder)).thenReturn(reminder);
        when(reminderMapper.toResponse(reminder)).thenReturn(response);
    ...
    void testUpdateReminder_Success() {
        ...
        ReminderResponse response = new ReminderResponse(1L, "Updated Reminder", "Activity Name", request.dateTime());
    ...
    void testUpdateReminder_MissingData_ThrowsException() {
        ...
        Exception exception = assertThrows(IllegalArgumentException.class, () -> {
            reminderService.update(1, request);
        });
    ...
    void testUpdateReminder_PastDate_ThrowsException() {
        ...
        Exception exception = assertThrows(IllegalStateException.class, () -> reminderService.update(1, request));
    ...
    void testCancelUpdate_DoesNotSaveChanges() {
        ReminderRequest request = new ReminderRequest(1L, "Updated Reminder", 100, LocalDateTime.now().plusDays(2));
        Reminder existingReminder = new Reminder();
    ...
    void testUpdateReminder_NotFound_ThrowsException() {
        ...
        Exception exception = assertThrows(ResourceNotFoundException.class, () -> reminderService.update(1, request));
    ...
    void testDeleteReminder_PastDate_ThrowsException() {
        ...
        Exception exception = assertThrows(IllegalStateException.class, () -> reminderService.delete(1));
    ...
    void testDeleteReminder_NotFound_ThrowsException() {
        ...
        Exception exception = assertThrows(ResourceNotFoundException.class, () -> reminderService.delete(1));
    ...
    void testGetById_Success() {
        Reminder reminder = new Reminder();
        reminder.setDateTime(LocalDateTime.now().plusDays(1));
        ReminderResponse response = new ReminderResponse(1L, "Test Reminder", "Activity Name", reminder.getDateTime());
    ...
    @Test
    @DisplayName("CP80 - Obtener recordatorio por id inexistente")
    void testGetById_NotFound_ThrowsException() {
        when(reminderRepository.findById(1)).thenReturn(Optional.empty());

        Exception exception = assertThrows(ResourceNotFoundException.class, () -> reminderService.getById(1));
        assertEquals("Recordatorio no encontrado", exception.getMessage());
    }

    // Tests para métodos adicionales

    @Test
    @DisplayName("CP81 - Obtener todos los recordatorios")
    void testGetAll_Success() {
        List<Reminder> reminders = List.of(new Reminder(), new Reminder());
        List<ReminderResponse> responses = List.of(
                new ReminderResponse(1L, "Reminder 1", "Activity 1", LocalDateTime.now().plusDays(1)),
                new ReminderResponse(2L, "Reminder 2", "Activity 2", LocalDateTime.now().plusDays(2))
        );
    ...
    @Test
    @DisplayName("CP82 - Obtener recordatorios para una semana")
    void testGetRemindersForWeek_Success() {
        ...
        List<ReminderResponse> responses = List.of(
                new ReminderResponse(1L, "Reminder 1", "Activity 1", LocalDateTime.now().plusDays(1)),
                new ReminderResponse(2L, "Reminder 2", "Activity 2", LocalDateTime.now().plusDays(3))
        );

        when(reminderRepository.findByDateTimeBetween(
        ...
        List<ReminderResponse> result = reminderService.getRemindersForWeek(startDate);

        assertNotNull(result);
        assertEquals(2, result.size());
        verify(reminderRepository).findByDateTimeBetween(
                startDate.atStartOfDay(),
                endDate.atTime(23, 59)
        );
    }
